...
if "student_id" not in ss:
    ss.student_id = None  # set via login
...
def login_panel():
    ...
    with st.form("student_login_form"):
        student_id = st.text_input("Student ID", value=ss.get("student_id") or "")
        submitted = st.form_submit_button("Login")
        if submitted:
            if not student_id.strip():
                st.error("Please enter a Student ID")
            else:
                ss.student_id = student_id.strip()
                st.success("Logged in")
                st.rerun()


def load_assignment(assignment_id: int):
    assignment = ss.homework_server.get_assignment(
        assignment_id, include_questions=True, include_image_bytes=False
    )
    ss.current_assignment = assignment
    # Get or create active submission
    submission = ss.homework_server.get_or_create_active_submission(
        ss.student_id, assignment_id
    )
    ss.submission = submission
    # Load existing attempts to compute attempts used
    answers = (
        ss.homework_server.get_submission_answers(submission["id"]) if submission else {}
    )
...
def grade_and_save(answers: Dict[int, str], finalize: bool = False):
    ...
    for q in questions:
        ...
        ok = ss.homework_server.record_answer_attempt(
    ...
    if graded_results:
        for r in graded_results:
            st.write(
                f"Q{r['question_id']} attempt {r['attempt']} → grade {r['grade']}: {r['feedback']}"
            )
...
"""
        ...
        ss.homework_server.mark_submission_completed(
            submission_id=submission["id"], overall_score=percent, summary=summary
        )
...
def take_assignment():
    ...
    st.caption(
        f"Submission ID: {submission['id']} — Started: {submission['started_at']}"
    )
    ...
    image_qs = ss.homework_server.get_assignment_questions(
        assignment['id'], include_image_bytes=True
    )
    ...
    for q in assignment.get("questions", []):
        ...
        answer = st.text_area(
            "Your answer", key=f"ans_{qid}", value=ss.answers_draft.get(qid, "")
        )
...
if not ss.student_id:
    ...
    with st.sidebar:
        choice = st.radio(
            "Navigate", ["assignments", "take"], index=0 if ss.page == "assignments" else 1
        )
        if choice != ss.page:
            ss.page = choice
            st.rerun()
