...
class DatabaseManagerPSQL:
    """
    Unified PostgreSQL database manager that combines all database operations.
    ...
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(DatabaseManagerPSQL, cls).__new__(cls)
    ...
    def _configure_database(self):
        """Configure and return a PostgreSQL database connection."""
        load_dotenv()
        host = os.getenv("POSTGRES_HOST", "dhost")
    ...
    @property
    def mydb(self):
        """Return the underlying PostgreSQL connection object, or None if not connected."""
        return self._mydb

    def get_connection(self):
        """Return an active connection, attempting to reconnect if needed."""
        try:
            if self._mydb and not self._mydb.closed:
                return self._mydb
            print("🔄 Reconnecting to PostgreSQL database...")
            self._mydb = self._configure_database()
            return self._mydb
        except Exception as exc:
            print(f"❌ Error getting PostgreSQL database connection: {exc}")
            return None
    ...
    def close_connection(self):
        ...
        try:
            ...
            print(f"❌ Error closing PostgreSQL database connection: {exc}")
    ...
    def create_user(self, user_data: Dict[str, Any]) -> Optional[int]:
        ...
        if not mydb:
            print("❌ No PostgreSQL database connection available")
            return None
    ...
    def authenticate_user(self, username: str, password: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            cursor = mydb.cursor(row_factory=dict_row)
            cursor.execute(
    ...
    def update_password(self, user_id: int, new_password: str) -> bool:
        ...
        try:
            ...
            # Direct update for password_hash since it's not in allowed_fields
            mydb = self.get_connection()
            if not mydb:
                print("❌ No PostgreSQL database connection available")
                return False
    ...
    def get_or_create_active_submission(self, student_id: int, assignment_id: int):
        ...
        try:
            ...
            cursor = mydb.cursor()
            cursor.execute(
                "INSERT INTO submissions (student_id, assignment_id, started_at, status) VALUES (%s, %s, %s, 'in_progress') RETURNING id",
                (student_id, assignment_id, datetime.now()),
            )
            mydb.commit()
    ...
    def mark_submission_completed(self, submission_id: int, overall_score: float, summary: str) -> bool:
        ...
        try:
            ...
            cursor.execute(
                ...
                (datetime.now(), overall_score, summary, submission_id),
            )
            mydb.commit()
            cursor.close()
            return True
    ...
    def upload_image(self, image_bytes, image_extension=None, content_type=None):
        ...
        try:
            ...
            sql = "INSERT INTO images (image_data, image_extension, created_at, file_size, content_type) VALUES (%s, %s, %s, %s, %s) RETURNING id"
            val = (image_bytes, image_extension, created_at, file_size, content_type)
            mycursor.execute(sql, val)
            mydb.commit()
            image_id = mycursor.fetchone()[0]
            mycursor.close()
            return image_id
        except Exception as e:
    ...
    def get_image(self, image_id):
        ...
        try:
            ...
            mycursor = mydb.cursor(row_factory=dict_row)
...
UserServer = DatabaseManagerPSQL
HomeworkServer = DatabaseManagerPSQL
ImageServer = DatabaseManagerPSQL
